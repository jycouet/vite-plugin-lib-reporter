...
import { formatSize } from "./formatString";
  ...
  return {
    ...
    async moduleParsed(module) {
      if (isInteresing(module.id)) {
        ...
        [...module.importedIds, ...module.dynamicallyImportedIds].forEach(
          (id) => {
        ...
        info[module.id].missing = [
          ...new Set([...info[module.id].missing, ...missing]),
        ];
      }
    },
    ...
    async writeBundle({ dir: outDir }, output) {
      ...
      if (outDir?.endsWith("client")) {
        ...
        for (const key in info) {
          ...
          output.forEach((o) => {
            o.removedExports.forEach((r) =>
              removedExports.push({ id: key, name: r })
            );
        ...
        addToLimit(
          ...
          "nbFiles",
          oriNbFiles,
          config.limit?.source?.nb_file_max
        );
        ...
        addToLimit(
          ...
          "minified",
          oriMinified,
          config.limit?.source?.minified_max
        );
        addToLimit(
          "source",
          "compressed",
        ...
        addToLimit(
          ...
          "size",
          optSize,
          config.limit?.treeshaked?.size_max
        );
        addToLimit(
          "treeshaked",
          "minified",
          optMinified,
          config.limit?.treeshaked?.minified_max
        );
        addToLimit(
          "treeshaked",
          "compressed",
        ...
        if (optMissing.length === 0) {
          ...
          log.error(
            `Missing in ${logYellow("includes")} and/or ${logYellow(
              "exculdes"
            )}: [${optMissing.map((c) => logRed(`'${c}'`)).join(", ")}]`
          );
        }
